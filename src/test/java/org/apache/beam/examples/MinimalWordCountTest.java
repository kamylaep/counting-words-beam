...
public class MinimalWordCountTest implements Serializable {
  ...
  private void assertData(String outputFileName, List<String> expectedWords) throws IOException {
    //TODO assert number of shards X number of files
    List<String> words = readWords(outputFileName);
    Assert.assertTrue(words.containsAll(expectedWords));
  }

  private List<String> readWords(String outputFileName) throws IOException {
    return Files.walk(Paths.get(options.getOutputPath()))
